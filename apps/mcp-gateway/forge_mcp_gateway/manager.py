"""MCP connection manager — the gateway's control plane (plan Task 1.12).

Registers :class:`~forge_contracts.MCPConnection` records, builds a per-connection
:class:`~forge_mcp.MCPGatewayClient` (each with its own transport), and shares a
single append-only audit log across all connections so the gateway can serve a
unified, redacted audit trail.

The transport is created via an injectable factory. The default factory returns
:class:`~forge_mcp.NullTransport`, which refuses live traffic — so no real MCP
network call ever happens unless a real transport is wired in explicitly.
"""
...
from forge_mcp import InMemoryAuditLog, MCPGatewayClient, NullTransport
from forge_mcp.audit import AuditSink
