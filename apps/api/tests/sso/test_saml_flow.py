...
class TestSpInitiatedLogin:
    ...
    def test_acs_happy_jit_and_link(self, client_factory, session_factory, idp_keypair: Keypair):
        install_config(session_factory, idp_keypair)
        client = client_factory(authenticated=False)

        # Round 1: SP-initiated login JIT-provisions dana.
        request_id = _request_id(_sp_login(client))
        b64 = sign_saml_response(build_saml_response(in_response_to=request_id), idp_keypair)
        ...
        request_id = _request_id(_sp_login(client))
        b64 = sign_saml_response(build_saml_response(in_response_to=request_id), idp_keypair)
        assert client.post("/auth/saml/acme/acs", data={"SAMLResponse": b64}).status_code == 302
        with session_factory() as session:
    ...
    def test_acs_audience_mismatch_400(self, client_factory, session_factory, idp_keypair):
        ...
        request_id = _request_id(_sp_login(client))
        b64 = sign_saml_response(
            build_saml_response(in_response_to=request_id, audience="https://other-sp.example"),
            idp_keypair,
        )
        response = client.post("/auth/saml/acme/acs", data={"SAMLResponse": b64})
        assert response.status_code == 400
    ...
    def test_acs_unknown_in_response_to_rejected(
        ...
        install_config(session_factory, idp_keypair)
        client = client_factory(authenticated=False)
        b64 = sign_saml_response(build_saml_response(in_response_to="_never_issued"), idp_keypair)
        response = client.post("/auth/saml/acme/acs", data={"SAMLResponse": b64})
        assert response.status_code == 400
...
class TestGroupRoleMappingViaAcs:
    def test_mapped_group_grants_admin_unmapped_does_not(
        ...
        client = client_factory(authenticated=False)
        b64 = sign_saml_response(
            build_saml_response(name_id="lead@acme.com", attributes={"groups": ["forge-admins"]}),
            idp_keypair,
        )
        assert client.post("/auth/saml/acme/acs", data={"SAMLResponse": b64}).status_code == 302
        b64 = sign_saml_response(
...
class TestDiscovery:
    def test_discover_routes_known_domain(self, client_factory, session_factory, idp_keypair):
        install_config(session_factory, idp_keypair)
        client = client_factory(authenticated=False)
    ...
    def test_discover_unknown_domain(self, client_factory, session_factory, idp_keypair):
        install_config(session_factory, idp_keypair)
        client = client_factory(authenticated=False)
        response = client.post("/auth/saml/discover", json={"email": "who@unknown.example"})
...
class TestAuditAndNonRegression:
    def test_login_audits_success_and_failure(self, client_factory, session_factory, idp_keypair):
        install_config(session_factory, idp_keypair, allow_idp_initiated=True)
        client = client_factory(authenticated=False)
        good = sign_saml_response(build_saml_response(), idp_keypair)
        assert client.post("/auth/saml/acme/acs", data={"SAMLResponse": good}).status_code == 302
        bad = encode_unsigned(build_saml_response())
        assert client.post("/auth/saml/acme/acs", data={"SAMLResponse": bad}).status_code == 400
        with session_factory() as session:
            rows = session.execute(
                select(AuditLog.action, AuditLog.result).where(AuditLog.workspace_id == WS_ID)
            ).all()
    ...
    def test_slo_endpoint_is_honestly_parked(self, client_factory, session_factory, idp_keypair):
        install_config(session_factory, idp_keypair)
        client = client_factory(authenticated=False)
