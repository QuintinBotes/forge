...
class FileSpecEngine:
    ...
    def spec_create(
        ...
        *,
        ...
        # Validate whichever key we end up with, allocated or supplied. An
        # allocated one can still collide: the ordinal comes from what is on
        # disk, so a store that has lost documents (an unmounted spec_root, a
        # restore, a manual edit) hands back a number already in use. Writing it
        # anyway folded the new spec into the existing one as another version —
        # a create and an update are not the same operation, and the overwrite
        # was silent.
        key = self._validate_new_key(spec_key(self._next_spec_number()) if key is None else key)
        # Deliberately no generated acceptance criteria. A criterion earns its
        # place by being checkable independently of the requirement; one that
        # reads "Implementation satisfies R1: <R1 verbatim>" restates the
        # obligation and cannot fail, while a dashboard counting 7/7 present
        # reports the spec as well specified. An empty list the author must fill
        # is the honest state, and the traceability projection already flags the
        # gap.
    ...
    def spec_plan(self, spec_id: uuid.UUID) -> SpecManifest:
        ...
        if not manifest.decisions:
            # A placeholder ADR is a prompt to the author, never a record. The
            # previous one asserted `status: accepted` over three fields that
            # recorded nothing ("Follow existing repository conventions" rules
            # nothing out), so an auditor reading the spec later would take the
            # architecture as settled — which is most misleading precisely when
            # the real questions are still sitting in `open_questions`.
            #
            # `proposed`, with the fields naming what is missing, reads as the
            ...
            manifest.decisions = [
                ADR(
                    ...
                    status="proposed",
                    context=(
                        "Placeholder generated by `spec plan`. No decision has been "
                        "recorded yet; replace this with the forces at play."
                    ),
                    decision=(
                        "TODO: state the decision taken, and the alternatives "
                        "rejected with the reason each was rejected."
                    ),
                    consequences=(
                        "TODO: state what this decision makes easy, what it makes "
                        "hard, and what it forecloses."
                    ),
                )
            ]
