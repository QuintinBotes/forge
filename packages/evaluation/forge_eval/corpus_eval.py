...
def format_eval_report(
    ...
    *,
    ...
    lines.append(
        ...
        "They supersede the deterministic `recall@5 = 1.000` headline in "
        "`docs/MORNING_REPORT.md` §4, which is a **wiring check only**."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "Re-reviewed the `forge_knowledge.sync` + `forge_eval.retrieval_eval` "
        'refutation flagged in `MORNING_REPORT.md` §6 ("1 refutation, '
        'repaired=false"). **Outcome: invalid/superseded.** The refutation was '
        "that the deterministic eval's 1.000 scores did not prove real retrieval "
        "quality — a *realism* gap, not a code defect in sync/eval. HARD-04 closes "
        "that gap by measuring the same pipeline on a real corpus with a learned "
        "embedder (this report). The sync ingestion path is unchanged and remains "
        "green; no code defect was surfaced. See the note in "
