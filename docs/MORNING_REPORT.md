...
5. **LangGraph swap** — agent runtime uses an in-house FSM graph engine (`packages/agent-runtime/forge_agent/graph.py`) rather than `langgraph.graph.StateGraph`; marked as the seam to swap once LangGraph is a build dependency.
